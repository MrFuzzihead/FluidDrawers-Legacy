...
- **GL state (revised post-test):** blend is enabled ONLY when `alpha < 1.0F` (translucent gaseous/lighter-than-air fluids); the common opaque case (water, lava) never touches blend — this fixed a rare white-flash GL leak seen during fill. `GL_CULL_FACE` is no longer toggled (the fluid box is convex; default back-face cull renders it from every exterior angle). `GL_LIGHTING` disable/restore and the lightmap (`setLightmapTextureCoords` + per-vertex `setBrightness`) save/restore remain. Disabling lighting stops the tint being double-darkened; the lightmap (texture unit 1) still applies via the texture env, so torch-brightness tracking works.
- **Light emission (post-test addition):** `TileTank.getFluidLightLevel()` returns the stored fluid's `getLuminosity()`; `BlockTank.getLightValue(IBlockAccess,x,y,z)` delegates to it so a lava tank emits light 15. `onStoredFluidChanged` calls `worldObj.func_147451_t(x,y,z)` (relight) only when luminosity changes. Unscaled by fill (OpenBlocks reference). See `docs/verification-log.md` Finding 1.
