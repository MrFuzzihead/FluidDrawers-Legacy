...
| 15    | Concealment key (optional polish)                          | ModItems.shroudKey → toggle `concealed` attribute (SimpleDrawerAttributes) in the held-item dispatcher; RenderTileTank already gates `renderFluid` on `isConcealed()`; NBT + sync already carry the attribute | 7, 10       | Concealment key toggles hiding the fluid (in-world TESR).                                                                                     |
...
- [x] The `concealed` attribute is NBT-persisted (`SimpleDrawerAttributes.serializeNBT`/`deserializeNBT` write/read `"concealed"`) and carried by the TE description packet (`TileTank.getDescriptionPacket` → `writeToNBT` → `writeToPortableNBT` writes `"Attributes"`), so the client TESR sees the toggle immediately and the state survives save/reload + sealed break/place.
- [x] Security-first ordering preserved (the `SecurityManager.hasAccess` guard runs before the held-item dispatch, so a non-owner cannot toggle concealment).
- [x] `./gradlew build` PASS (spotless + compile).

> **Status (2026-07-29):** Implemented + `./gradlew build` PASS (8s, spotless + compile clean). The attribute infrastructure (`concealed` field, `isConcealed()`/`setConcealed()`, NBT ser/des, render gate, sync) was already in place from Phases 4/5/7/11; Phase 15 only wires the `ModItems.shroudKey` toggle into the dispatcher. No new lang keys (the shroud key item's name/tooltip come from SD's lang; the branch is silent like the lock-key branch -- the visual fluid hide/show is the feedback). Manual `runClient` in-game verification pending. Dedicated-server gate N/A (no new sync/proxy/GUI code; reads already-synced TE state). See `docs/verification-log.md` and `docs/work-orders/phase15-concealment-key.md`.
