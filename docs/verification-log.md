...
- **Symptom:** with the tank in a hotbar slot, selecting any other slot that holds an item (bucket, pickaxe, another tank) makes the tank icon render darker; selecting an empty-hand slot renders it normal brightness. (A first lightmap-only fix did NOT resolve it — see "Fix history" below.)
- **Root cause (verified):** `ItemRenderer.renderItemInFirstPerson` does **two** things that both leak past the hand render into the HUD hotbar pass (the hotbar is drawn after the hand in the same frame): (1) `enableStandardItemLighting` (`ItemRenderer:272`, world-aligned fixed-function GL lights) and (2) sets the lightmap to the player's **ambient** world light (`ItemRenderer:287-290`). The leaked OpenGL light modulates the tank icon's vertex color (darkening it under the world-aligned lights), and the leaked ambient lightmap darkens the lightmap sampling. With an empty hand, the hand path leaves the GUI full-bright state, hence the symptom only appears while holding something. Because the custom `ItemRendererTank` is registered (`ClientProxy:33`), the GUI/hotbar path is `ForgeHooksClient.renderInventoryItem` → our `renderItem(INVENTORY)`.
- **Fix history:** a first attempt forced only the lightmap current coord (`setLightmapTextureCoords(240, 240)`, gated to INVENTORY) — but it did NOT resolve the darkening, proving the leaked **OpenGL light** (not just the lightmap) was the dominant darkener. The final fix adds a `fullBright` parameter to `renderInventoryFrame`: when true (INVENTORY/WAILA icon) it (a) disables fixed-function `GL_LIGHTING` (bypassing the leaked OpenGL-light darkening) and (b) bakes `FULL_BRIGHT` per-vertex via `tess.setBrightness` (bypassing the leaked lightmap). Verified vs decompiled `RenderBlocks.renderFaceYNeg`: the non-AO branch (lines 7306-7312) only does `addVertexWithUV` and does NOT call `setBrightness`, so the vertices inherit the Tessellator's current brightness — baking `FULL_BRIGHT` guarantees they sample the lightmap at (15,15). The `EQUIPPED*`/`ENTITY` types pass `fullBright=false` so the held/dropped item still inherits the ambient lightmap (dark at night), preserving the Finding-3 design and avoiding the "glows near-white in darkness" regression.
- **Sealed-tank follow-up (regression then fix):** the `fullBright` fix initially only covered the frame+glass via `renderInventoryFrame`. The sealed tank's tape overlay is drawn in a **separate Tessellator batch after `renderInventoryFrame`'s `glPopAttrib`**, so it lost the blend/`fullBright` setup (blend restored to off, lighting restored to the leaked hand state). This re-introduced BOTH the opaque-glass and the darker-while-holding symptoms for sealed tanks: the tape icon's transparent pixels rendered opaque (no blend) and the leaked light darkened them, masking the fixed frame+glass behind. Fixed by wrapping the tape batch in the same `glPushAttrib(COLOR_BUFFER_BIT | DEPTH_BUFFER_BIT | GL_LIGHTING_BIT)` + `glEnable(GL_ALPHA_TEST)`/`glAlphaFunc(GL_GREATER, 0.1F)`/`glEnable(GL_BLEND)`/`OpenGlHelper.glBlendFunc(770, 771, 1, 0)` + (when `inventoryFullBright`) `glDisable(GL_LIGHTING)` + `tess.setBrightness(FULL_BRIGHT)` block, with `glColor4f(1,1,1,1)` reset + `glPopAttrib` after. The redundant lightmap save/restore was removed (per-vertex `setBrightness` supersedes the current-coord approach for both the frame+glass and the tape).
...
### Finding 7 — Sealed tank item showed only the tape (no fluid/overlays), unlike the in-world tank
- **Symptom:** a sealed tank (e.g. with lava) placed in-world shows the fluid + overlays through/with the packing tape, but the sealed tank ITEM (inventory/hotbar/hand/dropped) showed only the frame + tape — no fluid, no upgrade trim, no lock/claim/void overlays.
- **Root cause:** the non-WAILA sealed branch in `ItemRendererTank.renderItem` rendered only `renderInventoryFrame` (frame+glass) + a manual tape batch. It did NOT reconstruct the tile or call `renderFluidForItem` / `renderInventoryOverlays` (those were only on the WAILA HUD path). So the sealed item hid its contents.
- **Fix (per user direction — item should mirror in-world):** the non-WAILA `hasTile` branch now mirrors the WAILA path — it reconstructs a transient `TileTank` from the `"Tile"` portable NBT, then renders `renderInventoryFrame` (with the `fullBright` param + vending-icon check) + `RenderTileTank.renderFluidForItem` + `BlockTankRenderer.renderInventoryOverlays`. `renderInventoryOverlays` already draws the **tape** when `tile.isSealed()` (verified: `BlockTankRenderer:354-356`), plus the storage-trim / lock-claim / void indicators — so the separate manual tape batch was removed entirely (it would have double-drawn the tape). The now-unused `Tessellator` import and the local `FULL_BRIGHT` constant were removed. Empty/unsealed items (no `"Tile"` NBT) still render frame+glass only. `worldObj` stays null; `readFromPortableNBT` + the render helpers touch only portable state (null-world-safe, same as the WAILA path).
...
5. **Sealed tank item shows contents:** seal a tank with lava (+ upgrades/keys) → the sealed ITEM in the hotbar/inventory/hand/dropped should now show the lava fluid + upgrade trim + lock/claim/void overlays + the tape (matching the in-world placed appearance), not just the tape. An empty/unsealed tank item still shows frame+glass only.
